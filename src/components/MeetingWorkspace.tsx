...
export function MeetingWorkspace({ sharedMeetingId }: { sharedMeetingId?: string }) {
  const { meetings, updateMeeting, templates, setTemplate, storageError } = useMeetingStore();
  const location = useSyncExternalStore(subscribeToUrl, () => window.location.pathname + window.location.search, () => sharedMeetingId ? "/share/" + sharedMeetingId : "/");
  const params = new URLSearchParams(location.split("?")[1] || "");
  const selectedMeetingId = params.get("meeting") || (location.startsWith("/share/") ? sharedMeetingId : null);
