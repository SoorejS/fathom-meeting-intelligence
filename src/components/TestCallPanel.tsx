...
import { FloatingNotetaker } from "./FloatingNotetaker";

export function TestCallPanel({ state, engine, onClose, onOpenMeeting, minimized, onMinimize, onRestore }: {
  minimized: boolean; onMinimize(): void; onRestore(): void;
  ...
  const button = "px-4 py-2.5 rounded-lg bg-cyan-400 text-slate-950 font-semibold text-sm hover:bg-cyan-300 disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white";
  const secondary = "px-4 py-2.5 rounded-lg border border-slate-600 text-sm text-slate-200 hover:bg-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400";
  const active = ["recording", "ending", "processing", "acquiring"].includes(state.phase);
  const label = state.phase === "recording" ? `${state.call?.captureMode === "microphone" ? "REC" : "SIM REC"} ${formatTime(state.elapsed)} · ${TEST_PARTICIPANTS.length} participants` : state.phase === "complete" ? "Meeting ready" : state.phase === "precall" ? "Set up test call" : state.phase === "permission" ? "Consent required" : state.phase === "acquiring" ? "Microphone permission" : state.phase === "processing" ? "Processing meeting" : state.phase === "ending" ? "Ending meeting" : state.phase === "joining" ? "Connecting" : state.phase === "declined" ? "Recording declined" : "Capture interrupted";
  return <FloatingNotetaker minimized={minimized} onMinimize={onMinimize} onRestore={onRestore} recording={state.phase === "recording"} label={label}
    onClose={active ? undefined : onClose}
    footer={state.phase === "recording" ? <button className="w-full px-4 py-2 rounded-lg bg-red-500 hover:bg-red-400 text-white text-sm font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white" onClick={() => { void engine.end(); }}>End Meeting</button> : undefined}>
        {state.phase === "precall" ? <>
          ...
          {state.phase === "recording" && <>
            <div className="flex items-center justify-between"><p aria-label="Elapsed recording time" className="text-3xl font-mono text-white">{formatTime(state.elapsed)}</p><span className="text-xs text-emerald-400">Consent approved · Connected</span></div>
